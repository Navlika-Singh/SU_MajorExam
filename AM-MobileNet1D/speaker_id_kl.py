...
if kl_file == 'none':
    ...
    checkpoint_load = torch.load(kl_file)
    MOBILENET_net_teacher = MobileNetV2(num_classes=class_lay)
    MOBILENET_net_teacher.cuda()
    MOBILENET_net_teacher.load_state_dict(checkpoint_load['MOBILENET_model_par'])
    MOBILENET_net_teacher.eval()
...
for epoch in range(begin_epochs, N_epochs):
    ...
    for i in tqdm(range(N_batches)):
        ...
        # ----- MobileNet -----
        with torch.no_grad():
            logits = MOBILENET_net_teacher(inp)


        pred=torch.max(pout,dim=1)[1]
        loss = cost(pout, lab.long())

        ## pout and logits already have log softmax applied

        loss += kl_criterion(pout, torch.exp(logits))
